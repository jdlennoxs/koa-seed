...
function outputLog(data, thrownError) {
    if (config_1.config.prettyLog) {
        if (data.statusCode < 400) {
            console.log(green(`${data.statusCode} ${data.method} ${data.url} = ${data.responseTime}ms`));
        }
        else {
            console.log(yellow(`${data.statusCode} ${data.method} ${data.url} = ${data.responseTime}ms`));
        }
        if (thrownError) {
            console.error(red(thrownError));
        }
    }
